...
const prettyOptions = {
  ...
  ignore: 'module,reqId,format,pid,hostname',
}
 ...
 *   ... do work ...
 *   end({ extra: 'fields' })  // 输出: msg="parse 完成" durationMs=123
 */
export function startTimer(label: string): (extra?: Record<string, unknown>) => void {
  const begin = Date.now()
  return (extra) => {
    const durationMs = Date.now() - begin
    logger.info({ label, durationMs, ...extra }, `${label} 完成`)
  }
}
