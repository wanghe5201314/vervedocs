...
function normalizeCatalogTitleName(elementList: IElement[]): string {
  return getTextFromElementList(elementList)
    .replace(/\u200B/g, '')
    .replace(/\s+/g, ' ')
    .trim()
}

export function getOrCreateTitleId(titleId?: string): string {
  return titleId || getUUID()
}

export function buildCatalogFromElementList(elementList: IElement[]): ICatalog | null {
  const titleItems: ICatalogItem[] = []

  const collectTitles = (elements: IElement[]) => {
    let index = 0
    while (index < elements.length) {
      ...
      if (element.type === ElementType.TITLE) {
        if (element.titleId && element.level) {
          const name = normalizeCatalogTitleName(element.valueList || [])
          if (name) {
            titleItems.push({
              id: element.titleId,
              name,
      ...
      if (element.titleId && element.level) {
        ...
        while (index < elements.length) {
          const titleElement = elements[index]
          if (!titleElement || titleElement.titleId !== titleId) {
            break
          }
          titleElementList.push(titleElement)
          index++
        }
        const name = normalizeCatalogTitleName(titleElementList)
        if (name) {
          titleItems.push({
            id: titleId,
            name,
      ...
      if (Array.isArray(element.valueList)) {
        collectTitles(element.valueList)
      }
      index++
    }
  }

  collectTitles(elementList)
  if (!titleItems.length) return null

  const catalog: ICatalogItem[] = []
  const stack: ICatalogItem[] = []
  for (const item of titleItems) {
    while (
      stack.length > 0 &&
      titleOrderNumberMapping[stack[stack.length - 1].level] >= titleOrderNumberMapping[item.level]
    ) {
      stack.pop()
    }
    if (stack.length === 0) {
      ...
      stack[stack.length - 1].subCatalog.push(item)
    }
    stack.push(item)
  }
...
interface IFormatElementListOption {
  ...
  laTexToSVG?: (laTex: string) => { svg: string; width: number; height: number }
}
...
export function formatElementList(
  ...
  if (
    ...
    elementList.unshift({
      ...
      listLevel: startElement?.listLevel,
      listIndent: startElement?.listIndent,
      listHanging: startElement?.listHanging,
      paragraphIndentLeft: startElement?.paragraphIndentLeft,
      paragraphSpacingBefore: startElement?.paragraphSpacingBefore,
      paragraphSpacingAfter: startElement?.paragraphSpacingAfter,
      paragraphFirstLineIndent: startElement?.paragraphFirstLineIndent
    })
  }
  let i = 0
  while (i < elementList.length) {
    ...
    if (el.type === ElementType.TITLE) {
      ...
      // 追加节点
      if (valueList.length) {
        const titleId = getOrCreateTitleId(el.titleId)
      ...
      (!el.type || (TEXTLIKE_ELEMENT_TYPE.includes(el.type) && el.type !== ElementType.PLACEHOLDER)) &&
      el.value?.length > 1
    ) {
      elementList.splice(i, 1)
    ...
    if (el.type === ElementType.LATEX && laTexToSVG) {
      const { svg, width, height } = laTexToSVG(el.value)
...
export function zipElementList(
  ...
  while (e < elementList.length) {
    ...
    if (
      ...
      // 将段落级属性转移到下一个元素，避免跳过时丢失
      const nextElement = elementList[e + 1]
      if (nextElement) {
        EDITOR_ROW_ATTR.forEach(attr => {
          if (
            element[attr] !== undefined &&
            nextElement[attr] === undefined
          ) {
            ;(nextElement as any)[attr] = element[attr]
          }
        })
      }
      e++
      ...
      if (element.type === ElementType.TABLE) {
        ...
        if (element.block?.type === BlockType.VIDEO) {
          ...
          if (src || srcdoc) {
            ...
            iframe.sandbox.add(...IFRAME_SANDBOX)
...
export function isParagraphSeparator(element: IElement): boolean {
  return element.value === ZERO && !element.listWrap
}

export function isFirstElementOfParagraph(index: number, elementList: IElement[]): boolean {
  return index === 0 || isParagraphSeparator(elementList[index - 1])
}

export function isTableElement(element: IElement): boolean {
