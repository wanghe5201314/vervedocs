import { ElementType, ZERO, TEXTLIKE_ELEMENT_TYPE, ControlComponent, EditorContext, IControlContext, IElement, IRangeStyle, IRange, IRangeElementStyle, IRangeParagraphInfo, RangeRowArray, RangeRowMap, getAnchorElement, isParagraphSeparator } from '@wanghe1995/docx-editor-schema'
type Draw = any
...
export class RangeManager {
  ...
  private get position() {
    return this.draw.getPosition()
  }

  private get historyManager() {
    return this.draw.getHistoryManager()
  }
  ...
  public setRangeStyle() {
    ...
    const isSubscribeRangeStyleChange =
      this.eventBus.hasSubscribers('rangeStyleChange')
    ...
    const hasSelection = !!this.getSelectionElementList()?.length
    ...
    const characterScale = curElement.characterScale ?? 100
    ...
    const paragraphColor = curElement.paragraphColor || null
    let paragraphFirstLineIndent = 0
    {
      const elementList = this.draw.getElementList()
      for (let si = ~endIndex ? endIndex : 0; si >= 0; si--) {
        const el = elementList[si]
        if (el.paragraphFirstLineIndent !== undefined) {
          paragraphFirstLineIndent = el.paragraphFirstLineIndent
          break
        }
        if (si > 0 && isParagraphSeparator(elementList[si - 1])) {
          break
        }
      }
    }
    ...
    let extension: unknown | null = curElement.extension ?? null
    if (type === ElementType.TABLE) {
      const table = {
        borderType: curElement.borderType ?? null,
        borderColor: curElement.borderColor ?? null,
        borderWidth: curElement.borderWidth ?? null,
        borderExternalWidth: curElement.borderExternalWidth ?? null
      }
      if (extension && typeof extension === 'object' && !Array.isArray(extension)) {
        extension = { ...(extension as Record<string, unknown>), table }
      } else {
        extension = { table }
      }
    }
