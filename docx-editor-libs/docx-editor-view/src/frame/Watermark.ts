...
import { FORMAT_PLACEHOLDER } from '@wanghe1995/docx-editor-schema'
import { WatermarkType } from '@wanghe1995/docx-editor-schema'
...
export class Watermark {
  ...
  public renderText(ctx: CanvasRenderingContext2D, pageNo: number) {
    ...
    if (repeat) {
      // 使用 OffscreenCanvas 进行水印渲染（性能更好）
      let temporaryCanvas: HTMLCanvasElement | OffscreenCanvas
      let temporaryCtx: CanvasRenderingContext2D | OffscreenCanvasRenderingContext2D
      if (typeof OffscreenCanvas !== 'undefined') {
        temporaryCanvas = new OffscreenCanvas(1, 1)
        temporaryCtx = temporaryCanvas.getContext('2d') as OffscreenCanvasRenderingContext2D
      } else {
