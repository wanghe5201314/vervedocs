import { IEditorOption, IElement, ElementType, RenderMode } from '@wanghe1995/docx-editor-schema'
import {
  PUNCTUATION_LIST,
  METRICS_BASIS_TEXT
...
import { ITextMetrics } from '@wanghe1995/docx-editor-schema'
...
export class TextParticle {
  ...
  private curCharacterScale: number
  ...
  constructor(draw: Draw) {
    ...
    this.text = ''
    this.curStyle = ''
    this.curCharacterScale = 100
    this.cacheMeasureText = new Map()
  }

  public static getCharacterScaleRatio(characterScale?: number): number {
    const nextCharacterScale = Math.round(characterScale ?? 100)
    return nextCharacterScale > 0 ? nextCharacterScale / 100 : 1
  }

  public static fillScaledText(
    ...
    const ratio = TextParticle.getCharacterScaleRatio(characterScale)
    if (ratio === 1) {
  ...
  public record(
    ...
    if (
      ...
      (element.characterScale ?? 100) !== this.curCharacterScale
    ) {
      this.complete()
      this._setCurXY(x, y)
    }
    ...
    this.curStyle = element.style
    this.curColor = element.color
    this.curCharacterScale = element.characterScale ?? 100
  }
