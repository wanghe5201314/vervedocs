...
import { TableOrder } from '@wanghe1995/docx-editor-schema'
...
export class TableTool {
  ...
  public render() {
    ...
    rowAddBtn.style.left = `${tableX + tableWidth / 2}px`
    rowAddBtn.style.top = `${tableY + tableHeight}px`
    rowAddBtn.style.transform = `translate(-50%, ${this.ROW_COL_QUICK_OFFSET * scale}px)`
    // 快捷添加行
    rowAddBtn.onclick = () => {
      this.position.setPositionContext({
        index,
