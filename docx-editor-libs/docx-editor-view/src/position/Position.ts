import { ListStyle, RowFlex, ElementType, VerticalAlign } from '@wanghe1995/docx-editor-schema'
...
import { deepClone, isRectIntersect } from '@wanghe1995/docx-editor-schema'
...
import { getIsBlockElement, isTableElement, isImageElement } from '@wanghe1995/docx-editor-schema'
        ...
        for (let c = 0; c < curRow.columnRowLists.length; c++) {
          ...
          if (colRowList.length > 0) {
            ...
            for (const colRow of colRowList) {
              index += colRow.elementList.length
            }
          }
          ...
          for (let t = 0; t < element.trList!.length; t++) {
            ...
            for (let d = 0; d < tr.tdList!.length; d++) {
              ...
              const drawRowResult = this.computePageRowPosition({
                ...
                startRowIndex: 0,
                startIndex: 0,
                startX: (td.x! + effectiveTdPadding[3]) * scale + tablePreX,
                startY: (td.y! + effectiveTdPadding[0]) * scale + tablePreY,
