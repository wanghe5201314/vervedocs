...
import DocxEditor, { ICatalogItem, IElement, PaperDirection, TableBorder, parseDocx } from '@wanghe1995/docx-editor-core'
...
const initEditor = async () => {
  if (editorContainer.value) {
    ...
    // 注册图表插件
    const { createChartPlugin } = await import('@wanghe1995/docx-editor-chart')
    const echarts = await import('echarts')
    editorInstance.use(createChartPlugin({ echarts }))

    // 注册 AI 插件
    const { createAIPlugin } = await import('@wanghe1995/docx-editor-ai')
        ...
        const action = await new Promise<string>((resolve) => {
          ...
          ;(async () => {
            try {
              ...
              const { EchartsChartRenderer } = await import('@wanghe1995/docx-editor-chart')
