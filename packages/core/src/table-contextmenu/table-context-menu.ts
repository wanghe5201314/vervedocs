...
const ICONS = {
  ...
  tableBorder: 'grid_on',
  borderAll: 'grid_on',
  borderExternal: 'border_outer',
  borderInternal: 'border_inner',
  borderDash: 'view_quilt',
  borderNone: 'grid_off',
  borderColor: 'palette',
  borderWidth: 'line_weight',
  cellAlign: 'format_align_center',
  ...
  delete: 'delete',
  deleteRow: 'delete',
  deleteTable: 'delete_outline',
  cancelMerge: 'unfold_less',
  merge: 'call_merge',
  split: 'call_split',
  selectAll: 'select_all',
  backgroundColor: 'format_color_fill',
  cellBorder: 'border_all',
  slash: 'gesture'
}
...
export class TableContextMenu {
  ...
  public show(x: number, y: number, element: IElement) {
    ...
    this._adjustMenuPosition(this.menuElement, x, y)
    ...
    return [
      {
        ...
        icon: ICONS.copy,
        shortcut: 'Ctrl+C',
        action: () => this.draw.getCanvasEvent().copy()
      },
      {
        label: '粘贴',
        icon: ICONS.paste,
      ...
      },
      {
        ...
        icon: ICONS.cut,
        shortcut: 'Ctrl+X',
        action: () => this.draw.getCanvasEvent().cut()
      },
      {
        divider: true
      },
  ...
  private _adjustMenuPosition(menu: HTMLDivElement, x: number, y: number): void {
    ...
    if (y + rect.height > viewportHeight) {
      menu.style.top = `${Math.max(10, viewportHeight - rect.height - 10)}px`
    }
  }
  ...
  private _pasteFromClipboard() {
    ...
    navigator.clipboard?.read?.().then(async (items: ClipboardItems) => {
