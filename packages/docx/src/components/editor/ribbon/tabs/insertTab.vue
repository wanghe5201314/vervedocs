<template>
  <div class="ribbon-tab-panel">
    ...
    <RibbonGroup title="插图">
      ...
      <RibbonButton icon="chart-bar" text="图表" title="插入图表" size="large" command="insertChart" />
    </RibbonGroup>
    ...
    <RibbonGroup title="链接">
      ...
      <a-dropdown :trigger="['click']">
        <RibbonButton icon="function-variant" text="公式" title="公式" size="large" has-arrow />
        <template #overlay>
          <a-menu @click="({ key }: any) => handleFormulaMenuClick(String(key))">
            <a-menu-item key="latex">
              <span class="mi"><VIcon name="function-variant" /><span>插入LaTeX公式</span></span>
            </a-menu-item>
            <a-sub-menu v-for="cat in formulaCategories" :key="'formula-' + cat.name">
              <template #title>
    ...
    <RibbonGroup title="分隔符">
      <a-dropdown :trigger="['click']">
        <RibbonButton icon="separator-page-break" text="分页" title="分页" size="large" has-arrow />
        <template #overlay>
          <a-menu @click="({ key }: any) => emit('command', key)">
            <a-menu-item key="pageBreak"><span class="mi"><VIcon name="separator-page-break" /><span>分页符</span></span></a-menu-item>
        ...
        <template #content>
          <a-card size="small" class="ribbon-popover-card" :bordered="false" :bodyStyle="{ padding: '8px' }">
            <div class="separator-list">
              <div class="separator-item" v-for="sep in separatorStyles" :key="sep.name" @click="handleInsertSeparator(sep)">
    ...
    <RibbonGroup title="页眉页脚">
      <a-dropdown :trigger="['click']">
        <RibbonButton icon="page-layout-header-footer" text="页眉和页脚" title="页眉和页脚" size="large" has-arrow />
        <template #overlay>
          <a-menu @click="({ key }: any) => emit('command', key)">
...
import { formulaCategories, separatorStyles } from '../../toolbar/index'
