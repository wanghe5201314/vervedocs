  ...
  <div class="ribbon-tab-panel">
    ...
    <RibbonGroup title="页面设置">
        ...
        <template #overlay>
          <a-card size="small" :bordered="false" class="ribbon-overlay-card" :bodyStyle="{ padding: '0' }">
            <div class="direction-panel">
              <div class="direction-item" @click="emit('command', 'paperDirection', 'vertical')">
                <div class="direction-icon vertical-icon"></div>
                <span>纵向</span>
              </div>
              <div class="direction-item" @click="emit('command', 'paperDirection', 'horizontal')">
      ...
      <a-dropdown :trigger="['click']" overlayClassName="gdocs-menu-popper gdocs-margin-popper">
        <RibbonButton icon="layout-margin" text="页边距" title="页边距" size="large" has-arrow />
        <template #overlay>
          <a-card size="small" :bordered="false" class="ribbon-overlay-card" :bodyStyle="{ padding: '0' }">
            <div class="margin-presets">
              <div class="preset-item" v-for="preset in marginPresets" :key="preset.name" @click="emit('command', 'setPaperMargin', preset.margins)">
        ...
        <template #overlay>
          <a-card size="small" :bordered="false" class="ribbon-overlay-card" :bodyStyle="{ padding: '0' }">
            <div class="bg-menu">
              ...
              <div class="bg-grid">
                <button v-for="c in bgColorPalette" :key="c" class="bg-color" :class="{ selected: selectedBgColor === c }" :style="{ backgroundColor: c }" @click="emit('command', 'setPaperBackground', c)"></button>
              </div>
            </div>
          </a-card>
        </template>
          ...
          <a-card size="small" class="watermark-card">
            ...
            <div class="wm-section">
              ...
              <div class="wm-preset-grid">
                <div class="wm-preset-item" v-for="preset in watermarkPresets" :key="preset.name" @click="emit('command', 'addWatermark', preset.options)">
...
import { paperSizes, marginPresets, bgColorPalette, watermarkPresets } from '../../toolbar/index'
