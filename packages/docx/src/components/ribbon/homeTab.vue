  ...
  <div class="ribbon-tab-panel">
    ...
    <VdRibbonGroup title="字体">
      <a-select
        ...
        size="small"
        style="width: 120px"
        @change="(v: any) => emit('font', String(v))"
      >
        <a-select-option
          v-for="f in fontList"
          :key="f.value"
      ...
      <a-select
        :value="sizeValueToLabel(currentSize)"
        size="small"
        style="width: 70px"
        @change="(v: any) => emit('size', sizeLabelToValue(String(v)))"
      >
        <a-select-option
          v-for="s in sizeList"
          :key="s.label"
      ...
      <a-dropdown :trigger="['click']">
        ...
        <template #overlay>
          <a-menu @click="({ key }: any) => handleCharacterScale(key)">
            <a-menu-item
              v-for="scale in CHARACTER_SCALE_OPTIONS"
              :key="'scale' + scale"
            >
              <span
                class="scale-check"
                :class="{ active: currentCharacterScale === scale }"
                >{{ currentCharacterScale === scale ? '✓' : '' }}</span
              >{{ scale }}%
            </a-menu-item>
            <a-menu-divider />
            <a-menu-item key="scaleCustom"
              ><span class="scale-check"></span>其他(M)...</a-menu-item
            >
          </a-menu>
        </template>
      </a-dropdown>
          ...
          >
            <div class="list-panel">
              ...
              <div class="list-grid bullet-grid">
                <button
                  class="list-cell list-cell-none"
                  @click="emit('bullet', null)"
                >
                  无
                </button>
                <button
                  v-for="b in BULLET_STYLES"
                  :key="b.style"
                  class="list-cell"
                  :title="b.label"
                  @click="emit('bullet', b.style)"
                >
                  <div class="list-preview">
                    <div class="list-preview-row" v-for="i in 3" :key="i">
                      <span class="list-symbol">{{ b.icon }}</span
          ...
          >
            <div class="list-panel">
              ...
              <div class="list-grid number-grid">
                <button
                  class="list-cell list-cell-none"
                  @click="emit('number', null)"
                >
                  无
                </button>
                <button
                  v-for="n in NUMBER_STYLES"
                  :key="n.style"
                  class="list-cell"
                  :title="n.label"
                  @click="emit('number', n.style)"
                >
                  <div class="list-preview">
                    <div class="list-preview-row" v-for="i in 3" :key="i">
                      <span class="list-num">{{ n.samples[i - 1] }}</span
                      ><span class="list-line"></span>
                    </div>
                  </div>
                </button>
            ...
            @click="
              ({ key }: any) =>
                emit(
                  'command',
                  'firstLineIndent',
                  Number(key) * INDENT_PX_PER_CHAR
                )
            "
          >
            <a-menu-item v-for="o in FIRST_LINE_INDENT_OPTIONS" :key="o.value">{{
              o.label
            }}</a-menu-item>
          </a-menu>
        </template>
    ...
    <VdRibbonGroup title="样式">
      <a-dropdown :trigger="['click']">
        ...
        <template #overlay>
          <a-menu
            @click="
              ({ key }: any) => emit('title', key === 'null' ? null : key)
            "
          >
...
/** 字号选项列表 */
const sizeList = EDITOR_SIZE_OPTIONS

/**
 * 处理字符缩放菜单点击，自定义项触发 characterScaleCustom 命令
 * @param key - 菜单项键值
 */
