...
export class DatePicker {
  ...
  private _setPosition() {
    ...
    const height = layout?.pages?.[0]?.rect?.height
      ?? this.draw.getOptions().pageHeight
      ?? 1123
    const pageGap = (this.draw.getOptions() as { pageGap?: number }).pageGap ?? 24
  ...
  public render(option: IRenderOption) {
    // 按需挂载到容器，避免初始化时就出现在页面上
    if (!this.dom.container.parentElement) {
      this.draw.getContainer().append(this.dom.container)
    }
