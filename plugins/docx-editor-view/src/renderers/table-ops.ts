import { ElementType, IElement, TableBorder, VerticalAlign } from '@vervedoc/docx-editor-schema'
import { ZERO } from '@vervedoc/docx-editor-schema'
import { TABLE_CONTEXT_ATTR } from '@vervedoc/docx-editor-schema'
import { TdBorder, TdSlash } from '@vervedoc/docx-editor-schema'
import { DeepRequired } from '@vervedoc/docx-editor-schema'
import { IEditorOption } from '@vervedoc/docx-editor-schema'
import { IColgroup } from '@vervedoc/docx-editor-schema'
import { ITd } from '@vervedoc/docx-editor-schema'
import { ITr } from '@vervedoc/docx-editor-schema'
import { cloneProperty, getUUID } from '@vervedoc/docx-editor-schema'
import {
...
import { TableParticle } from './table'
...
export class TableOperate {
  ...
  public insertTable(row: number, col: number) {
    ...
    const { defaultTrHeight, defaultTrMinHeight, tdPadding, defaultBorderColor } =
      this.options.table
    ...
    const baseTrHeight =
      Number.isFinite(Number(defaultTrHeight)) && Number(defaultTrHeight) > 0
        ? Number(defaultTrHeight)
        : 0
    const autoTrMinHeight = Math.max(
      defaultTrMinHeight,
      baseTrHeight,
      Math.ceil(this.options.defaultSize * 1.4 + tdPaddingHeight)
    )
    // colgroup
  ...
  public mergeTableCell() {
    ...
    const isPlaceholder = (el: IElement | undefined) =>
      !!el && el.value === ZERO && !el.type
    const anchorContextElement =
      anchorTd.value.find(el => !isPlaceholder(el)) ||
      anchorTd.value[0] ||
      ({
        value: ZERO,
        tableId: element.id,
        trId: curTrList[anchorTd.trIndex!]?.id,
        tdId: anchorTd.id
      } as IElement)
    for (let t = 0; t < rowCol.length; t++) {
      const tr = rowCol[t]
      for (let d = 0; d < tr.length; d++) {
        ...
        if (!isAnchorTd) {
          ...
          let startTdValueIndex = 0
          while (isPlaceholder(td.value[startTdValueIndex])) {
            startTdValueIndex++
          }
          const isAnchorAllPlaceholder =
            anchorTd.value.length > 0 && anchorTd.value.every(isPlaceholder)
          if (isAnchorAllPlaceholder && startTdValueIndex < td.value.length) {
            anchorTd.value.length = 0
          }
          for (let d = startTdValueIndex; d < td.value.length; d++) {
            const tdElement = td.value[d]
            if (isPlaceholder(tdElement)) continue
            cloneProperty<IElement>(
              TABLE_CONTEXT_ATTR,
              anchorContextElement,
              tdElement
            )
    ...
    if (!anchorTd.value.length || !isPlaceholder(anchorTd.value[0])) {
      const placeholder: IElement = { value: ZERO }
      cloneProperty<IElement>(TABLE_CONTEXT_ATTR, anchorContextElement, placeholder)
      anchorTd.value.unshift(placeholder)
    } else {
      const i = 1
      while (i < anchorTd.value.length) {
        if (isPlaceholder(anchorTd.value[i])) {
          anchorTd.value.splice(i, 1)
          continue
        }
        break
      }
    ...
    if (payload === TableBorder.EMPTY) {
      ...
      if (trList) {
        for (let t = 0; t < trList.length; t++) {
          ...
          for (let d = 0; d < tr.tdList.length; d++) {
            const td = tr.tdList[d]
            if (td.borderTypes) delete td.borderTypes
            if (td.borderStyle) delete td.borderStyle
          }
        }
      }
    }
  ...
  public tableBorderWidth(payload: number) {
    ...
    const originalElementList = this.draw.getOriginalElementList()
    const element = originalElementList[index!]
    const nextWidth = Number(payload)
    if (!Number.isFinite(nextWidth)) return
    if (nextWidth <= 0) {
      if (element.borderWidth === undefined) return
      delete element.borderWidth
    } else {
      if (element.borderWidth === nextWidth) return
      element.borderWidth = nextWidth
      if (
        element.borderExternalWidth !== undefined &&
        element.borderExternalWidth <= 0
      ) {
        delete element.borderExternalWidth
      }
      if (
        element.borderExternalWidth !== undefined &&
        element.borderExternalWidth === nextWidth
      ) {
        delete element.borderExternalWidth
      }
    }
  ...
  public tableBorderExternalWidth(payload: number) {
    const positionContext = this.position.getPositionContext()
    if (!positionContext.isTable) return
    const { index } = positionContext
    const originalElementList = this.draw.getOriginalElementList()
    ...
    const nextWidth = Number(payload)
    if (!Number.isFinite(nextWidth)) return
    const baseWidth = element.borderWidth ?? 1
    if (nextWidth <= 0 || nextWidth === baseWidth) {
      if (element.borderExternalWidth === undefined) return
      delete element.borderExternalWidth
    } else {
      if (element.borderExternalWidth === nextWidth) return
      element.borderExternalWidth = nextWidth
    }
    const { endIndex } = this.range.getRange()
    this.draw.render({
    ...
    tdList.forEach(td => {
      ...
      if (isSetBorderType) {
        ...
        td.borderStyle[payload] = {
          width: element.borderWidth ?? 0.5,
          color: element.borderColor || this.options.table.defaultBorderColor,
          style: element.borderType === TableBorder.DASH ? 'dashed' : 'solid'
        }
      } else {
        if (~borderTypeIndex) {
          td.borderTypes.splice(borderTypeIndex, 1)
        }
        if (td.borderStyle && td.borderStyle[payload]) {
          delete td.borderStyle[payload]
          if (!Object.keys(td.borderStyle).length) {
            delete td.borderStyle
          }
        }
      }
      // 不存在边框设置时删除字段
