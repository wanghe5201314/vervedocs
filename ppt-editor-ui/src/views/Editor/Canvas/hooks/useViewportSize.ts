...
export default (canvasRef: Ref<HTMLElement | undefined>) => {
  ...
  // 计算画布可视区域的位置
  // canvasPercentage 控制画布占容器空间的比例，100% = 填满容器（与 Office 行为一致）
  // BASE_SCALE = 0.95 表示画布基准大小为原尺寸的 95%
  ...
  const setViewportPosition = () => {
    ...
    if (canvasHeight / canvasWidth > viewportRatio.value) {
      const viewportActualWidth = canvasWidth * (canvasPercentage.value / 100) * BASE_SCALE
      ...
      const viewportActualHeight = canvasHeight * (canvasPercentage.value / 100) * BASE_SCALE
