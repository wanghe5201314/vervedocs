      ...
      <tbody>
        <tr v-for="(rowCells, rowIndex) in tableCells" :key="rowIndex">
          <td 
            ...
            v-contextmenu="(el: any) => contextmenus(el)"
          >
            <CustomTextarea 
...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    // 鍒ゆ柇鏌愪綅缃槸鍚VERS负鏃犳晥鍗曞厓鏍硷紙琚悎骞舵帀鐨勪綅缃級
    ...
    // 灏嗙劍鐐圭Щ鍔ㄥ埌涓嬩竴涓崟鍏冩牸
    // 当前行右边有单元格时，焦点右移
    // 当前行右边无单元格（已在行末），且存在下一行时，焦点移动至下一行行首
    // 当前行右边无单元格（已在行末），且不存在下一行（已在最后一行）时，新建一行并将焦点移动至下一行行首
    const tabActiveCell = () => {
    ...
    // 妫€鏌ユ槸鍚﹀彲浠ュ垹闄よ鍜屽垪锛氭湁鏁堢殑琛?鍒楁暟澶т簬1
...
table {
  ...
  user-select: none;
  --themeColor: #1a73e8;
  --subThemeColor1: #1a73e8;
  --subThemeColor2: #1a73e8;
}
table.theme {
  background-color: #fff;
}
table.theme tr:nth-child(2n) .cell {
  background-color: var(--subThemeColor1);
}
table.theme tr:nth-child(2n + 1) .cell {
  background-color: var(--subThemeColor2);
}
table.theme.row-header tr:first-child .cell {
  background-color: var(--themeColor);
}
table.theme.row-footer tr:last-child .cell {
  background-color: var(--themeColor);
}
table.theme.col-header tr .cell:first-child {
  background-color: var(--themeColor);
}
table.theme.col-footer tr .cell:last-child {
  background-color: var(--themeColor);
}
tr {
  height: 36px;
